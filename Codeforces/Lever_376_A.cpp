...
void sarthak()
{
    ...
    ll weight_l = 0, weight_r = 0;

    for (int i = 0; i < sz; i++)
    {
        ll add = abs(pivot - i) * (s[i] - '0');
        if(s[i] >= '1' && s[i] <= '9')
        {
            if(i < pivot)   weight_l += add;
            else weight_r += add;
        }
    }    

    if(weight_l > weight_r)     cout << "left";
    else if(weight_r > weight_l)    cout << "right";
    else cout << "balance";   
}
