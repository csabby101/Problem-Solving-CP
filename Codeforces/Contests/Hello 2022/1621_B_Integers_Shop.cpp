// problem link : https://codeforces.com/contest/1621/problem/B
...
void sarthak()
{
    ...
    int l_min = INT_MAX, r_max = 0, cost_l = INT_MAX, cost_r = INT_MAX, total = INT_MAX;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        else if (r == r_max && c < cost_r)
            cost_r = c;

        if (l == l_min && r == r_max && c < total)
            total = c;

        cout << min(total, cost_r + cost_l) << endl;
    }
}
