...
void sarthak()
{
    ...
    int gm, gM, lm, lM, checker = x, maxcount;
    // gm = global minima in the set, lm = local minima or immediate left
    // gM = global maxima in the set, lM = local maxima or immediate right
    ...
    for (int i = 3; i < n; i++)
    {
        ...
        if((x > gm && x < lm) || (x < gm && checker > gm && checker < gM))
        {
            cout << "yes";
            return;
        }
    
        else if ((x < gM && x > lM) || (x > gM && checker > gm && checker < gM))
        {
            cout << "yes";
            return;
        }
